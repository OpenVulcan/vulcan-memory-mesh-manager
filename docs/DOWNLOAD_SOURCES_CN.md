...
初始调研日期：2026-09-23。以下一至五节保留当时的设计与探测记录，不代表当前发行状态；2026-09-25 正式发行后的完整文件验证见第六节。这里的“国内镜像”特指面向国内用户的 GitHub Release 下载代理，不要求 OpenVulcan 自建对象存储。未在不同省份、运营商测量速度和长期稳定性。
...
管理器下载成功后，TUI 再独立展示 VMM 源：`GitHub 官方`、`ghproxy.net`、`gh-proxy.org`、`ghfast.top`、`自定义 GitHub 代理前缀`。引导源和 VMM 源可不同；用户选择的 VMM 源用于后续升级，并在首页可查看与修改。选择源是显式操作，下载失败时给出重试或切换选择，不静默改用另一来源。用户已明确本轮“镜像”指 GitHub 下载代理；自建静态文件镜像不在首版范围。
...
版本清单记录产品 ID、tag、源提交、平台、文件名、字节数、SHA-256、签名 key ID 与协议版本。下载前固定产品、版本与平台，下载后核对字节数、摘要、VMM 包内逐文件清单和可执行文件 `-version-json`。经第三方代理取得的资产必须与签名清单记录的字节一致；保留旧版本清单，避免更新过程中读到半套资产。
...
初始调研结论：预置 GitHub 官方、`ghproxy.net`、`gh-proxy.org`、`ghfast.top` 四个选项；后三者明确标为第三方 GitHub 代理，均须安装时动态检测。用户还可输入自己的 HTTPS GitHub 代理前缀并检测。没有已核实的 OpenVulcan 自有国内基址，初始调研未对三个代理做整包或中国内地多网络验收。

## 六、正式发行后的完整下载验收

2026-09-25 已公开 [VMM v0.1.1](https://github.com/OpenVulcan/vulcan-memory-mesh/releases/tag/v0.1.1) 和 [VMMM v0.1.2](https://github.com/OpenVulcan/vulcan-memory-mesh-manager/releases/tag/v0.1.2)。两个产品分别发布十六项与十一项资产，官方匿名下载均已逐字节核验。管理器已实施独立 Ed25519 清单和固定引导摘要，首次安装包只展示具备完整安装契约的 VMM 版本。

| 第三方代理 | 管理器 v0.1.2 Windows 文件，14,242,880 字节 | VMM v0.1.1 Windows 完整包，206,259,510 字节 |
| --- | --- | --- |
| `ghproxy.net` | 全文件下载与 SHA-256 通过 | 六十秒限制内仅收到 44,907,296 字节，超时；不计为完整下载通过 |
| `gh-proxy.org` | 全文件下载与 SHA-256 通过 | 全文件下载与 SHA-256 通过 |
| `ghfast.top` | 全文件下载与 SHA-256 通过 | 全文件下载与 SHA-256 通过 |

实际管理器文件摘要为 `28aafc1a5fe791596a02d5a762abd0048338067788b948ede8cb6fb4fafcade4`；VMM 包摘要为 `cf3e177aa68c8ffb595447c641600544538bdaaa64d97ca1593b37cb04d5da70`。这些结果只代表本次主机和时刻，不能推断中国内地所有网络均可用。

公开 Windows PowerShell 5.1 引导已实际下载并进入 TUI；官方源探测后返回菜单，三个预置代理和自定义入口均保留。`v0.1.2` 按来源稳定 ID 合并局部探测结果，已补回归覆盖返回、切源与自定义来源；`v0.1.0`、`v0.1.1` 旧资产保持不变，首装使用最新修复版。
