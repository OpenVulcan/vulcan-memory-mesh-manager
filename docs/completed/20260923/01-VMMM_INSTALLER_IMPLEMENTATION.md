...
状态：已完成。2026-09-25 完成合并、正式发布和公开安装验收。本文保留最初设计、每次发现与历史待办；最终范围、五轮审核及限制以末尾执行变更总结为准。
...
1. 核心调整：发现卸载已注销系统服务后，如果 PATH 收据缺失导致后续步骤失败，Linux 与 macOS 再次注销不存在的服务会拒绝执行，使用户修复收据后仍无法重试。现在锁内先查询服务状态；真实状态为未安装时继续剩余卸载步骤，不重复注销。连续无问题审核计数归零。
2. 文件变更：修改 `internal/controller/controller.go`、`internal/controller/controller_test.go` 与本计划；无新增或删除文件。
3. 关键逻辑：仅在已保存登记中的服务文件完整时运行其状态命令，明确的 `not-installed` 才跳过注销；状态无法核实时停止，不推测系统服务已消失。测试用实际安装状态与缺失 PATH 收据复现第一次失败，恢复收据后验证第二次完成，且系统服务只注销一次。测试假服务的注销状态与真实平台语义对齐。
4. 验证与边界：使用真实 VMM 标准布局的本机 Go 1.25.0 全量四百零五项测试、`go vet ./...` 和差异检查通过。管理器 `e056265` 的推送运行 `35898599456` 与拉取请求运行 `35898605012` 已在五个平台通过；本次修复仍待提交后原生复验。Certum 证书继续办理，正式发行门禁保持。

### 服务登记失败补偿复审

1. 核心调整：服务注销及自启切换原先只按用户请求反向调用系统命令；若服务本就不存在，登记写入失败会凭空重新注册；若自启设置已是目标值，登记写入失败会反向修改真实策略。现先读取并验证真实状态，补偿仅恢复操作前存在的设置。连续无问题审核计数重新归零。
2. 文件变更：修改 `internal/controller/controller.go`、`internal/controller/controller_test.go` 与本计划；无新增或删除文件。
3. 关键逻辑：注销不存在的原生服务时只清理过期管理登记，不再重复执行原生注销；登记保存失败时也不创建旧系统服务。自启开关在操作前验证状态值及 Windows 启动类型一致性，补偿依据操作前真实自启值；服务注销回滚用实际观察到的自启设置，而非可能过期的管理登记。测试覆盖缺失服务、已达目标的自启设置以及矛盾状态拒绝。
4. 验证与边界：使用真实 VMM 标准布局的本机 Go 1.25.0 全量四百零七项测试、`go vet ./...` 和差异检查通过。前一提交 `0c6a774` 的推送运行 `35899246590` 与拉取请求运行 `35899252410` 均在五个平台通过；本次补偿修复仍待提交后原生复验。Certum 正式发行门禁仍保持。

### 安装结果事件可靠性复审

1. 核心调整：安装完成状态由程序文件、配置、原生运行状态和健康探测共同判断，但进度事件占满缓冲区时，最终成功、失败或取消事件仍可能被无声丢弃。首次修复终止事件后，再次发现安装快照、已校验包信息等结果数据同样可能丢失；现保证状态结果和终止结果都能投递。发现问题后连续无问题审核计数归零。
...
1. 核心调整：按 VMM `EmbeddingConfig` 与 `RerankConfig` 的真实字段结构复核发现，快捷向导替换整个 `embedding` 或 `rerank` YAML 段会删除用户在高级编辑器设置的额度、批大小、密钥故障转移或 `top_n`。进一步跟踪 VMM 节点选路发现，保留旧 Embedding 节点又会使新顶层密钥失效。因此只保留不覆盖新密钥的高级字段，明确清空旧节点；连续无问题审核计数重新归零。
...
在服务原生启动策略与 PATH 撤销修复、VMM 发行工作流说明修正之后重新计数。以下五轮依次进行，均未发现需要继续修改的代码问题。

| 轮次 | 复核范围与事实依据 | 验证结果 |
| --- | --- | --- |
| 1 | 发行身份、下载源、引导脚本及草稿门禁：核对 Ed25519 内置公钥验签、产品与标签绑定、压缩包字节数和 SHA-256、HTTPS 重定向约束，以及两仓库只创建草稿的工作流。 | 管理器发行专项二十一项、VMM 发行专项十一项 Python 测试通过。 |
| 2 | 配置 schema、五种存储映射、数据库路径、供应商快捷配置与凭据引用：按真实 VMM 配置命令和字段结构复核，高级字段保留、旧节点清空及变更存储身份拒绝均与源码一致。 | 管理器六项真实 VMM 集成及配置定向测试、VMM 两项配置命令定向测试通过；全量四百一十四项管理器测试已通过。 |
| 3 | Linux/macOS 任意本机服务账户、Windows 服务策略、服务启停和三平台 PATH 所有权：检查原生状态补偿、权限边界、失败收据保留及重试入口。 | 服务、PATH 和安装库共七十五项测试，三项定向竞态测试通过；管理器提交 `451ead7` 的推送及拉取请求五平台 CI 均通过。 |
| 4 | 下载后安装、真实成功判定、升级回退和卸载重试：确认首次安装经过运行启动与健康检查，文件和配置核对通过后才最终写入完成标记；失败不报告成功。 | 安装完成、健康失败、重装和回退共十二项定向竞态测试通过。 |
| 5 | TUI 流程、CLI 操作、多语言文案和实施文档：确认语言与自定义源选择、先暂存包后配置、权威校验后确认、已安装操作及中英文错误提示。 | TUI 与 CLI 共四十三项测试、管理器 `go vet ./...` 和两仓库差异检查通过。 |

以上为证书到位前的五轮审核记录，当时正式签名和公开发行尚未完成。后续证书取得、签名接入与真实发行验收进度见下文；本计划继续区分代码完成、签名产物验证和公开安装验收。

### 正式发行流水签名接入与真实依赖验收

1. 核心修复与调整概述：证书已经可用，两个产品仓库的正式发行工作流均接入组织机密驱动的 SimplySign 云签名和最终 Linux 文件的 OpenPGP 分离签名，保留跨平台 Ed25519 清单。固定公开证书身份、GPG 公钥及 SimplySign 安装包身份；签名后才生成最终摘要。默认仅验证模式完整构建与签名后保存 Actions 产物，实际发行模式继续生成草稿。经用户授权，另行修复并公开 SQLite `v0.1.7` 与带相同签名的控制器 `v0.2.4`。
2. 📂文件变更清单：两个产品仓库新增 `scripts/sign_release.ps1`、`scripts/release_gpg.py`、`scripts/test_release_gpg.py`、`scripts/signing-policy.json`、`docs/release-gpg-public.asc`、`docs/release-signing_CN.md`；修改各自发行工作流和说明。管理器修改打包验证、工作流回归测试、持续集成及本计划；VMM 修改 `scripts/release.py`、发行专项测试、发行指南、持续集成工作流、`internal/app/storage_snapshot_test.go`，以及四个依赖安装或打包入口、固定摘要表及对应回归。无生产代码文件删除。
3. 💻关键代码调整详情：Windows 必须由固定 Certum 证书签名并带可信时间戳；VMM 四个程序和三个动态库全部纳入签名，更新原生库清单摘要后才打包。Linux 使用隔离密钥环签名，再用全新纯公钥密钥环验签；缺失签名、篡改和错误身份均拒绝。管理器旧有 Bash 内嵌文本缩进与缺少条件结束标记已修复并加入真实语法回归。VMM 修复快照测试目录权限，补齐安装真实依赖后的全量测试；更新 SQLite 与控制器十项已验证摘要。打包前固定输入集合与摘要，验收后拒绝原输入变更，ZIP/TAR 不收录验收产生的日志或数据库；测试夹具使用规范路径，覆盖 macOS 目录别名与 Windows 短路径差异。
4. ⚠️遗留问题与注意事项：管理器正式产品运行 `35986546656` 成功，构建源码为 `62749422077b5e574c89a794747ead3d58bc7cd4`，十一项资产已完成下载后的 Windows 运行、签名、时间戳、两份 GPG、Ed25519 和全部摘要复验。VMM 最终运行 `36002737086` 的七个任务全部通过，构建源码为 `cb3e653359e759fea51dfeb030a81ba8d267fb81`，十六项资产已完成独立签名、摘要与包内容复验，并由管理器生产解包器成功接收全部五平台包。同提交三轮五平台普通持续集成全部通过。早期单次 Windows 暂存测试失败未复现，根因仍未确认；已修复诊断输出缺失，未放宽断言或超时。原工作区已有配置未覆盖，控制器原目录的未提交开发内容未混入发行。

SQLite 与控制器上游均已完成原生五平台回归、连续五轮审核、资产下载复验及公开发行，未发布软件包注册表版本。SQLite 上游包未新增 Certum 签名，VMM 完整包中的 SQLite 动态库已签名；控制器的 Windows 程序与动态库、四份 Linux 压缩包均已签名。上游独立计划已分别归档。

本阶段签名接入和实际产物验证已完成。VMM 与管理器仍为草稿 PR，尚未合并或公开产品新版本；旧 VMM `v0.1.0` 不可覆盖。公开引导脚本的全新安装验收须在完整产品发行后执行，因此整体安装器计划仍保留在本目录。真实外部数据库与模型供应商接入按部署环境验收，不把本次包验收算作外部服务验收。

### 签名与发行增量连续五轮复核

在包内容修复及跨平台夹具规范路径修正后，基于 VMM `cb3e653` 重新计数。以下五轮由本任务自行依次执行，未使用子代理，均未发现新的待修复代码问题。证书到位前的五轮记录保持为独立历史证据。

| 轮次 | 复核重点 | 事实与验收证据 |
| --- | --- | --- |
| 1 | Windows 凭据处理、固定安装器、精确证书身份、时间戳和子进程边界 | 三仓库签名脚本语法通过，动态口令标准向量通过；证书选择、超时、环境清理与日志脱敏逐项核对，公开策略一致。 |
| 2 | Linux 与清单信任根、最终字节签名、错误身份及篡改拒绝 | 共用 GPG 公钥及公开身份一致，原生 Unix 临时密钥拒绝测试通过；最终文件两份 GPG 与 Ed25519 使用公开信任独立复验通过。 |
| 3 | 标签绑定、验证模式、权限、草稿与上传后回验 | 两仓库工作流静态检查通过；管理器发行脚本二十四项本机测试中二十一项通过、三项原生 Unix 用例由原生 CI 覆盖。固定公钥不可由下载源替换，正式模式保留草稿，公开旧版本未修改。 |
| 4 | 上游锁所有权、控制器内嵌依赖、固定摘要与完整打包顺序 | SQLite 显式拒绝未获锁状态；控制器固定修复提交，协议与接口未变。VMM 二十二项发行及宿主依赖测试在本机完成，两个 Unix 用例由原生任务覆盖；最终五平台真实依赖测试与三轮普通 CI 全部通过。 |
| 5 | 最终产物、安装器接收与完成声明 | VMM 十六项资产与 VMMM 十一项资产均完成下载复验；VMM 五个包各含八十九项负载及一份收据，全部经管理器生产解包器验收。七个 Windows 文件签名与实际版本身份通过，包内无验收日志或数据库；文档明确区分已公开上游版本与产品草稿。 |

详细运行链接、失败历史与五个平台最终摘要见 [正式发行签名](../../release-signing_CN.md)。后续文档与双语注释补充没有改变以上验收的签名、打包或产品运行逻辑。

### 公开首装发现的 Windows 引导缺陷与修复发行

两个 PR 已合并，VMM `v0.1.1` 与 VMMM `v0.1.0` 已完成正式签名、全部资产下载复验并公开。随后实际执行 Windows 首装发现两项之前未被模拟测试覆盖的问题：发布页 `irm | iex` 入口受到 BOM 和 PowerShell 5.1 响应编码影响；Windows 模板实际拼接出的下载 URL 缺少 `github.com`。管理器和 VMM 程序资产的签名、版本与包接收验收均通过，但不能将其代替引导脚本的实际执行。

本次修复步骤：保留兼容 Windows PowerShell 5.1 的带 BOM 文件，文档优先使用 `irm -OutFile` 后执行，PowerShell 7 管道入口明确移除 BOM；补全 Windows 官方域名。新增回归直接解析并执行生成脚本的真实 URL 赋值，覆盖官方、三个预置代理和自定义前缀；已在 PowerShell 5.1 与 7 分别复现旧实现失败。按公开版本不可覆盖原则发布管理器 `v0.1.1`，不修改旧标签或资产。修复后重新进行首装验收和连续五轮增量审核，再完成计划归档。

实际 TUI 返回路径进一步确认：控制器只发回被探测的一项来源，界面却把它当作完整目录替换，导致其他预置代理消失。原模拟控制器没有发回这一真实字段，故旧状态迁移测试未覆盖。修复将结果按稳定来源 ID 合并，保留未探测选项；模拟器改为真实的局部结果，并新增“探测、返回、切源”和自定义源保留回归。旧实现已明确复现四个来源缩减为一个，新实现的 TUI 测试通过。修复纳入 `v0.1.2`，待最终正式验收后完成连续五轮记录。

## 最终执行变更总结（2026-09-25）

### 一、核心修复与调整概述

独立管理器、五平台单文件发行、PS1/sh 引导、中文/英文 TUI、全部 YAML 字段与规则资产编辑、五种存储配置及检索算法说明、供应商向导、配置检查均已交付。已实现服务与命令行模式、启停重启、自启、PATH、升级回退、修复和保留数据卸载。用户拒绝持久化事务恢复的决定已落实：通过真实文件、配置和运行健康判断完成，未完成状态允许重新安装。

实际 Certum 与 GPG 签名接入 GitHub 托管运行器，两个产品保持独立 Ed25519 信任。已授权合并并公开 VMM `v0.1.1` 与 VMMM `v0.1.2`；上游 SQLite `v0.1.7` 和控制器 `v0.2.4` 已公开。引导域名缺失及局部探测覆盖源目录的问题均由实际公开安装发现、回归复现并修复，不覆盖已公开标签或资产。

### 二、📂文件变更清单

- 新增：`cmd/vmmm`、`internal` 下的下载、验签、解包、配置桥接及编辑、凭据、安装、控制、服务、PATH、自安装、TUI、国际化和状态模块及测试；五平台 CI 与发行工作流；引导、打包、Certum、GPG、Ed25519 及回归脚本；公开信任文件、README、实施与使用文档。空仓起步的完整文件清单见合并 PR #1。
- 修改：既有 `.gitignore`；后续在已新增代码上修正供应商字段保留、权限与补偿、结果事件、包边界、`scripts/install.ps1`、`scripts/test_generate_bootstrap.py`、`internal/tui/model.go`、模拟控制器、来源事件契约及相关回归。
- 删除：无生产源码删除。本计划由 `docs/plan/` 迁移到当前归档路径，README 与内部文档链接随之更新。

### 三、💻关键代码调整详情

所有配置语义以所下载 VMM 的 schema 和命令结果为准；YAML 节点编辑保留注释及未知字段，敏感字段使用环境变量引用。服务支持显式本机账户，模式切换在停机后核对及调整受管目录归属。安装仅在权威校验、首次运行和健康探测通过后写入完成标记；异常可以重装，清理不冒充成功。

发行按精确提交和不可变标签生成，最终 Windows 签名字节及 Linux GPG 签名完成后才固定清单；引导内嵌对应 SHA-256。下载源使用 HTTPS 明确前缀及稳定 ID，探测事件仅更新对应条目，保留其余来源。TUI 新回归在旧实现失败、修复后通过；全量 Go 测试与静态检查通过，最终代码五平台 CI `36030664973` 全部成功。引导和发行专项二十四项 Python 测试通过（本机跳过两项原生 Unix 用例，对应原生流水覆盖），TUI 与 CLI 四十四项通过。

公开管理器十一项、VMM 十六项资产全部以匿名 URL 完整下载复验。公开 Windows PowerShell 5.1 引导完成管理器更新、进入中文 TUI、探测返回、下载 VMM、配置、安装、真实健康以及命令行启停重启。默认卸载后十五个配置与数据文件逐一摘要不变，程序和登记移除。没有把构建通过代替安装成功；未调用真实收费模型端点。

### 四、⚠️遗留问题与注意事项

计划内交付及正式发行已完成。当前首装使用 VMMM `v0.1.2` 与 VMM `v0.1.1`；两次管理器旧发行保留不可变资产，并在说明中指向修复版。工作流保持先生成草稿并复验，再按授权公开的步骤。

本机实测覆盖 Windows TUI 和原生本地存储；Linux/macOS 的原生包、服务及 PATH 由五平台 CI 验收，未宣称完成这些平台的人工 TUI 首装。真实 PostgreSQL/ParadeDB 和供应商账户按用户部署环境验收。macOS 按用户要求不签名、公证。代理可用性动态变化，本机三个代理均通过管理器全文件校验；VMM 大包的 `ghproxy.net` 六十秒探测超时，其余两个完整通过。历史单次 Windows CI 暂存失败根因未确认，后续多轮同提交及正式流水均通过，记录没有删除。详见 [正式发行签名](../../release-signing_CN.md) 和 [下载源调研](../../DOWNLOAD_SOURCES_CN.md)。

### 最终公开修复版连续审核

以下审核由本任务独立依次执行，未调用子代理。从最后一次来源列表代码修复 `071aa98` 后重新计数，前面的失败及历史五轮不计入本次连续结果。

| 轮次 | 范围 | 证据与结果 |
| --- | --- | --- |
| 1 | 来源局部事件、返回、切源、自定义稳定 ID 和过期操作事件 | 沿真实控制器单项输出及 TUI 合并入口复核，选中来源与安装计划同步，旧操作 ID 提前拒绝；回归和 TUI/CLI 四十四项通过。未发现新问题。 |
| 2 | Windows 引导 URL、编码和公开文档命令 | 真实赋值回归覆盖 PowerShell 5.1/7 与全部来源；二十四项发行专项测试通过，本机两个 Unix 用例跳过；公开 5.1 文件入口实际打开 TUI。未发现新问题。 |
| 3 | 不可变标签、五平台产物、签名、公开下载与代理 | 正式流水与五平台 CI 通过；十一项资产、Certum、GPG、Ed25519、引导再生成及 Go 构建身份通过；官方匿名下载和三个代理管理器全文件摘要一致。未发现新问题。 |
| 4 | 真实首装完成判定、配置与凭据、运行管理和卸载保留 | 实际 TUI 安装完成，CLI 健康为 ok；停止、启动、重启成功，默认卸载十五个文件摘要不变；重复启动已运行实例的拒绝符合既有明确契约。未发现新问题。 |
| 5 | 最终公开身份、说明、设计归档与变更边界 | 两个 PR 均为已合并；远端标签解析至已验证提交，最新发行非草稿且资产数量正确。文档十项产品摘要、源码身份、相对链接及归档路径检查通过；两仓库差异检查通过，最后增量仅文档。未发现新问题。 |

最后代码修复后已连续五轮未发现新的待修复问题，审核计数完成。已将四项执行总结写入本计划后归档；历史失败、外部部署边界和平台证据范围均保留。
