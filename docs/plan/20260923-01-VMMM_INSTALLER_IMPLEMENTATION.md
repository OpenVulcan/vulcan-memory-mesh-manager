...
现有文件事务保存备份，但替换清单、旧登记和服务补偿状态仅存于内存；进程退出后无法重建。这不满足第一阶段的断电恢复验收，不能以普通异常回滚测试代替。

建议将恢复日志放在管理员控制的管理状态根，记录协议版本、事务标识、三类根目录、旧安装登记、旧运行状态、服务与路径变更意图，以及每个文件的旧摘要、新摘要、备份位置和推进阶段。凭据值不写入日志，私有备份遵循原权限限制。每次破坏性替换前先持久化意图并同步；文件替换后再推进记录。

重新进入管理器须先检查未完成事务，持有同一安装锁并停止仍运行的候选进程；逐项核实目录边界、文件类型、归属与摘要后逆序恢复文件、凭据、登记和系统服务。发生并发修改或身份不一致时保留备份并停止恢复，禁止自动覆盖用户数据。恢复完成后才允许启动、升级或卸载。

验收必须通过子进程强制退出覆盖备份前、备份后、推广后、登记写入后、服务变更后和最终提交后，并覆盖重复恢复、路径替换、损坏日志和并发编辑。该协议改变安装状态与启动恢复架构，已依仓库重大决策规则请求用户确认；在答复前不实施依赖此决策的代码。
...
终端验收复审发现此前以字符数量而非显示列截断文本，中文和宽表情会引起实际换行超限，长输入也看不到末尾。现复用已锁定的 Bubble Tea 显示依赖按字素及显示列计算，说明文字完整换行，输入区域横向滚动到末尾。新增中文、组合字符、表情、窄终端和长输入回归；本机含真实运行时校验的三百三十三项测试通过。健康检查提交的五平台持续集成全部通过。断电恢复仍待协议确认，最终连续审核计数保持零。
