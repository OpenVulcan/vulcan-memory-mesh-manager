...
1. 核心调整：安装完成状态由程序文件、配置、原生运行状态和健康探测共同判断，但进度事件占满缓冲区时，最终成功、失败或取消事件仍可能被无声丢弃。现保证每次操作恰好投递一个终止结果；发现问题后连续无问题审核计数归零。
2. 文件变更：修改 `internal/controller/controller.go`、`internal/controller/completeness_test.go`、`internal/state/state.go` 与本计划；无新增或删除文件。
3. 关键逻辑：普通进度继续使用非阻塞投递，终止事件在缓冲区已满时替换最早的一条进度，再关闭事件流。回归测试预先填满十六个事件槽，分别运行成功、失败和取消操作，确认每次恰好收到对应终止事件。修正数据根目录注释，明确安装登记位于独立控制状态根目录。
