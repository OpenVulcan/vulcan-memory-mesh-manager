...
Linux 独立账户服务烟测继续发现 VMM 默认日志目录位于管理员持有的程序包内，服务无法写入。VMM 已新增可选 `logging.directory` 并在服务注册前检查可写性；VMMM 服务模式自动写入所选账户数据根下的 `logs`，高级配置仍可显式修改。原生验收与最终五轮审核待修复后的 CI 完成，连续无问题轮数维持零。
