...
4. 选择命令行或服务运行方式、服务自启动策略，以及是否将永久安装的 `vmmm` 加入 PATH。高级配置编辑器按当前 VMM 的 `config schema --json` 展示字段，展开已有数组元素的具体路径，支持标量、对象和数组编辑，并保留 YAML 的注释与顺序。长字段列表随选中项滚动；编辑结构化 YAML 时按 `Ctrl+O` 换行，按 `Enter` 暂存字段。敏感字段在列表中脱敏，只接受 `${环境变量名}` 引用；包含敏感子字段的父级集合保持脱敏，支持用环境变量引用整体编辑，也可通过具体子字段修改。供应商快捷向导用于单供应商配置，保存时替换该用途的配置（LLM/重排为单路由），页面会明确提示；保留已有多路由与节点设置请使用高级编辑器。
5. 每次修改配置都会使上次校验及写入预览失效。VMM 的真实配置与规则资产校验通过后，确认页提供配置写入差异预览：逐字段展示已保存用户覆盖与候选覆盖的新增、修改和删除，支持滚动查看长值；秘密值始终隐藏，规则文件以内容摘要展示变化，凭据写入仅列变量名。此预览描述用户覆盖文件的写入，不代表系统底座与环境变量的最终合并来源。确认后才提交安装。
...
管理器不使用事务恢复日志，也不在重启后自动恢复安装步骤。已有安装登记只保存一个完成标记，全部步骤成功后才标记完成；打开管理器会核对受管程序文件的摘要及必要配置是否存在。标记未完成、文件缺失或损坏时显示“安装未完成 / 需要修复”，可选择“重新安装 / 修复”重新下载签名包。正常安装但未启动的实例显示已安装、已停止。
