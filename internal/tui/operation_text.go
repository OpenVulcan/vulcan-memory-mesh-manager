...
var operationChinese = map[string]string{
	...
	"Manager PATH change could not be rolled back; inspect PATH and retry removal if an ownership record exists":        "管理器 PATH 变更未能撤销；请检查 PATH，如有所有权记录则重试移除",
	"Windows service is disabled in Services; change its startup type to Manual before changing registration with vmmm": "Windows 服务已被设为禁用；请先在服务管理器中改为手动，再用 vmmm 修改服务登记",
