...
func (m *Model) renderHome() []string {
	...
	lines := []string{
		...
		m.label("管理器版本：", "Manager version: ") + valueOrDash(snapshot.ManagerVersion),
		m.label("VMM 版本：", "VMM version: ") + valueOrDash(snapshot.VMMVersion),
		m.label("下载源：", "Download source: ") + valueOrDash(snapshot.SourceID),
		m.label("存储：", "Storage: ") + string(snapshot.Storage),
		m.label("运行：", "Runtime: ") + boolState(snapshot.Running, m.label("运行中", "running"), m.label("未确认运行", "not confirmed running")),
		m.label("服务：", "Service: ") + valueOrDash(m.operationText(snapshot.ServiceState)),
		m.label("开机自启：", "Autostart: ") + boolState(snapshot.AutoStart, m.label("启用", "enabled"), m.label("关闭", "disabled")),
